...
class ClaimRevisionService:
    ...
    def propose(self, claim_id, new_statement, new_status, rationale, evidence_refs=(), actor="system"):
        ...
        self.db.execute("""INSERT INTO claim_revisions
            (id,claim_id,previous_statement,new_statement,previous_status,new_status,
             rationale,evidence_refs,revised_by,created_at)
            VALUES (?,?,?,?,?,?,?,?,?,?)""",
            (i,claim_id,claim["statement"],new_statement,claim["status"],new_status,
             rationale,json.dumps(list(evidence_refs),sort_keys=True),actor,now()))
    ...
    def approve(self, revision_id, reviewer):
        ...
        with self.db.transaction() as con:
            ...
            con.execute("UPDATE claims SET statement=?,status=? WHERE id=?",
                        (rev["new_statement"],rev["new_status"],rev["claim_id"]))
            con.execute(
