...
class CostControl:
    ...
    def record(self, correlation_id, amount, provider, model, purpose, actor="system", company_id="hds", metadata=None):
        ...
        with self.db.transaction() as con:
            ...
            updated=con.execute("UPDATE budgets SET spent_amount=spent_amount+?,updated_at=? WHERE id=? AND status='ACTIVE' AND spent_amount+?<=limit_amount",(amount,now(),row_dict["id"],amount))
            if updated.rowcount != 1:
                raise BudgetExceeded("cost would exceed budget under concurrent spending")
