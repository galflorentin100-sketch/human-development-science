...
class BudgetRequired(Exception): pass
class CostControl:
    ...
    def authorize(self, estimated_cost, company_id="hds"):
        if not isinstance(estimated_cost,(int,float)): raise ValueError("estimated_cost must be numeric")
        if estimated_cost < 0: raise ValueError("estimated_cost cannot be negative")
        budget=self.active_budget(company_id)
        if not budget: raise BudgetRequired("model/API spend is blocked until an active budget exists")
        remaining=float(budget["limit_amount"])-float(budget["spent_amount"])
        if estimated_cost > remaining: raise BudgetExceeded("requested spend exceeds remaining budget")
    ...
    def reserve(self, correlation_id, amount, provider="unknown", model="unknown", purpose="reserved", actor="system", company_id="hds", metadata=None):
        ...
        with self.db.transaction() as con:
            ...
            budget_row=cur.fetchone(); budget=self._row_dict(cur,budget_row) if budget_row is not None else None
            if not budget: raise BudgetRequired("no active budget")
            cur=con.execute("SELECT * FROM budgets WHERE id=? AND status='ACTIVE'",(budget["id"],)); row=cur.fetchone()
            if row is None: raise BudgetRequired("active budget disappeared")
            b=self._row_dict(cur,row); event_id=str(uuid4()); ts=now()
            ...
            cur=con.execute("SELECT * FROM cost_events WHERE id=?",(event_id,)); return self._row_dict(cur,cur.fetchone())
    def settle(self, correlation_id, actual_amount, provider, model, purpose, actor="system", company_id="hds", metadata=None):
        ...
        with self.db.transaction() as con:
            cur=con.execute("SELECT * FROM cost_events WHERE correlation_id=?",(correlation_id,)); row=cur.fetchone()
            ...
            if delta>0:
                ...
                con.execute("UPDATE budgets SET spent_amount=CASE WHEN spent_amount+? < 0 THEN 0 ELSE spent_amount+? END,updated_at=? WHERE id=? AND status='ACTIVE'",(delta,delta,now(),r["budget_id"]))
            ...
            cur=con.execute("SELECT * FROM cost_events WHERE id=?",(r["id"],)); return self._row_dict(cur,cur.fetchone())
    ...
    def release_stale_reservations(self,max_age_minutes=60,company_id="hds"):
        ...
        released=[]
        with self.db.transaction() as con:
            cur=con.execute("SELECT * FROM cost_events WHERE status='RESERVED' AND created_at<?",(cutoff,))
            rows=cur.fetchall()
            for row in rows:
                r=self._row_dict(cur,row)
                con.execute("UPDATE budgets SET spent_amount=CASE WHEN spent_amount-? < 0 THEN 0 ELSE spent_amount-? END,updated_at=? WHERE id=? AND status='ACTIVE'",(r["amount"],r["amount"],now(),r["budget_id"]))
                changed=con.execute("UPDATE cost_events SET status='RELEASED',metadata=? WHERE id=? AND status='RESERVED'",(json.dumps({"released":True,"reason":"stale_reservation_recovery"}),r["id"]))
                if getattr(changed,"rowcount",1)==1: released.append(r["correlation_id"])
        return released
    def set_budget(self, limit_amount, company_id="hds", currency="USD", period="LIFETIME"):
        ...
        if existing:
            self.db.execute("UPDATE budgets SET limit_amount=?,updated_at=? WHERE id=?",(limit_amount,now(),existing["id"])); return self.db.one("SELECT * FROM budgets WHERE id=?",(existing["id"],))
        budget_id=str(uuid4()); self.db.execute("INSERT INTO budgets(id,company_id,limit_amount,spent_amount,currency,period,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)",(budget_id,company_id,limit_amount,0,currency,period,"ACTIVE",now(),now())); return self.db.one("SELECT * FROM budgets WHERE id=?",(budget_id,))
